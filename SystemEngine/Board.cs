...
namespace SystemEngine
{
    public class Board
    {
        ...
        private readonly Cell[,] r_CellArray;

        public Board(int i_NumOfRows, int i_NumOfCols)
        {
            m_NumOfRows = i_NumOfRows;
            m_NumOfCols = i_NumOfCols;
            r_CellArray = new Cell[m_NumOfRows, m_NumOfCols];
            initializeBoard();
        }
        ...
        public void ClearAllPawnsOnCells()
        {
            for(int i = 0; i < m_NumOfRows; i++)
            {
                for(int j = 0; j < m_NumOfCols; j++)
                {
                    r_CellArray[i, j].PawnOnCell = null;
                }
            }
        }
